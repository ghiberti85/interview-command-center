...
export async function callAI(messages, system, token, signal) {
  const headers = { "Content-Type": "application/json" };
  if (token) headers["Authorization"] = `Bearer ${token}`;

  // Always enforce a 30s hard timeout; combine with optional caller signal
  ...
  try {
    ...
    if (e.name === "AbortError" || e.name === "TimeoutError") throw new Error("aborted");
    throw e;
  }
